  ...
  useEffect(() => {
    ...
    const initFav = async () => {
      try {
        ...
        if (!u || !u.phone) return;
        const customerRef = doc(db, 'Restaurant', 'orderin_restuarant_7', 'customers', u.phone);
  ...
  useEffect(() => {
    if (!item && id) {
      const fetchItem = async () => {
        try {
          const querySnapshot = await getDocs(collection(db, "Restaurant", "orderin_restuarant_7", "menu"));
