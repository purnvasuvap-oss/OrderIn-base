...
function App() {
  ...
  const [alertSelectedItemId, setAlertSelectedItemId] = useState("");
  const [showAlertItemOptions, setShowAlertItemOptions] = useState(false);
  ...
  const filteredAlertItemOptions = alertItemOptions
    .filter(item => {
      const term = alertItemName.trim().toLowerCase();
      if (!term) return true;
      return (
  ...
  const handleAlertItemInput = (value) => {
    setAlertItemName(value);
    setAlertSelectedItemId("");
    setShowAlertItemOptions(true);

    const exactItem = alertItemOptions.find(item =>
      item.name.toLowerCase() === value.trim().toLowerCase() ||
      item.id.toLowerCase() === value.trim().toLowerCase()
    );

    if (exactItem) {
      setAlertSelectedItemId(exactItem.id);
      setAlertItemName(exactItem.name);
      setAlertItemCategory(exactItem.category);
    }
  };

  const handleSelectAlertItem = (item) => {
    setAlertItemName(item.name);
    setAlertSelectedItemId(item.id);
    setAlertItemCategory(item.category);
    setShowAlertItemOptions(false);
  };
        ...
        <div className="Inventory-stats-row">
          <div className="Inventory-card-stat Inventory-card-good">
            <div className="Inventory-card-top">
              <CheckCircle className="Inventory-card-icon" size={24} aria-hidden="true" />
          ...
          <div className="Inventory-card-stat Inventory-card-low">
            <div className="Inventory-card-top">
              <AlertTriangle className="Inventory-card-icon" size={24} aria-hidden="true" />
          ...
          <div className="Inventory-card-stat Inventory-card-verylow">
            <div className="Inventory-card-top">
              <XCircle className="Inventory-card-icon" size={24} aria-hidden="true" />
      ...
      {showAlertsModal && (
        <div className="Inventory-modal-overlay">
          <div className="Inventory-modal-box">
            ...
            <div className="Inventory-modal-body">
              <div className="Inventory-alert-picker">
                <label className="Inventory-alert-label" htmlFor="alert-item-name">Item Name</label>
                <input
                  id="alert-item-name"
                  type="text"
                  ...
                  value={alertItemName}
                  autoComplete="off"
                  onFocus={() => setShowAlertItemOptions(true)}
                  onChange={(e) => handleAlertItemInput(e.target.value)}
                />
                {showAlertItemOptions && (
                  <div className="Inventory-alert-dropdown">
                    {filteredAlertItemOptions.length > 0 ? (
                      filteredAlertItemOptions.map(item => (
                        <button
                          type="button"
                          key={item.id}
                          className="Inventory-alert-option"
                          onMouseDown={(e) => {
                            e.preventDefault();
                            handleSelectAlertItem(item);
                          }}
                        >
                          <span>
                            <strong>{item.name}</strong>
                            <small>{item.category || "No category"}{item.quantity ? ` · ${item.quantity}` : ""}</small>
                          </span>
                          {item.status && <em>{item.status}</em>}
                        </button>
                      ))
                    ) : (
                      <div className="Inventory-alert-empty">No matching inventory item</div>
                    )}
                  </div>
                )}
              </div>
              ...
              <button className="Inventory-btn Inventory-full Inventory-red" onClick={async () => {
                ...
                const inventoryCollection = collection(db, "Restaurant", "orderin_restaurant_2", "inventory");
                const selectedDocId = alertSelectedItemId || alertItemName;

                try {
                  const existingSnapshot = await getDocs(inventoryCollection);
                  const existingDoc = existingSnapshot.docs.find(docSnap => {
                    const item = docSnap.data();
                    return docSnap.id === selectedDocId || item.name === alertItemName;
                  });
                  const existingItem = existingDoc?.data();
                  if (!existingDoc || !existingItem) {
                    alert("Item not found in inventory.");
                    return;
                  }
                  const finalDocRef = doc(inventoryCollection, existingDoc.id);
                  const currentQty = parseFloat(existingItem.quantity.split(' ')[0]);
