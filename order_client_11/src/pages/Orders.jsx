...
function ManualOrderModal({ isOpen, onClose, menuItems, onOrderCreated }) {
  ...
  const orderSubtotal = selectedItems.reduce((sum, item) => {
    const itemPrice = Number(String(item.price || 0).replace(/[^0-9.-]+/g, "")) || 0;
    const itemQty = Number(item.quantity) || 1;
    return sum + itemPrice * itemQty;
  }, 0);
  const orderTax = orderSubtotal > 0 ? Math.ceil(orderSubtotal / 100) : 0;
    ...
    <div className="manual-order-overlay" onClick={onClose}>
      <div className="manual-order-modal" onClick={(e) => e.stopPropagation()}>
        <div className="modal-header">
          <div>
            <span className="modal-eyebrow">Counter order</span>
        ...
        <div className="modal-content">
          ...
          <div className="menu-selection-section">
            <h3><span>2</span> Add Items from Menu</h3>
          ...
          <div className="selected-items-section">
            <h3><span>3</span> Selected Items ({selectedItems.length})</h3>
            <div className="selected-items-list">
              {selectedItems.length === 0 ? (
                <div className="manual-empty-state">Add menu items to build the order</div>
        ...
        <div className="modal-footer">
          <div className="manual-order-total">
            ...
            {orderSubtotal > 0 && <small>Subtotal ₹{orderSubtotal} + tax ₹{orderTax}</small>}
          </div>
