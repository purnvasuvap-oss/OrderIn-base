...
import { deductInventoryForOrderItems } from "./inventoryRecipeService";
 ...
 * Path: /Restaurant/orderin_restuarant_6/customers/<phone_number>/pastOrders
...
/**
 * Accept an order and, best-effort, deduct any recipe-mapped ingredients
 * from inventory (see inventoryRecipeService.deductInventoryForOrderItems).
 * The two callers of this (Orders.jsx, KitchenDisplay.jsx) both already have
 * `order` and a `{name: menuItem}` lookup on hand, so this is the single
 * place that sequences accept -> deduct instead of duplicating it per page.
 *
 * Inventory deduction failures are swallowed (logged only) — accepting the
 * order is the primary action and must not be blocked or rolled back by a
 * secondary stock-bookkeeping problem.
 */
export const acceptOrderAndDeduct = async (order, menuItemsByName, restaurantId = RESTAURANT_ID) => {
  await acceptOrder(order.phoneNumber, order.id, restaurantId);

  let deductionSummary = [];
  try {
    deductionSummary = await deductInventoryForOrderItems(order.items, menuItemsByName);
  } catch (error) {
    console.error("Non-fatal: recipe inventory deduction failed for order", order.id, error);
  }
