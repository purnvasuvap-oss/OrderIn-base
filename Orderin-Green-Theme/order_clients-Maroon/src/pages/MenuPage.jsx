...
const MenuPage = () => {
    ...
    try {
      ...
      itemsToProcess = itemsToProcess.map((item) => ({
        ...
        recipe: Array.isArray(item.recipe)
          ? item.recipe
              .map((row) => ({
                inventoryItemName: String(row.inventoryItemName || "").trim(),
                quantity: Number(row.quantity) || 0,
                unit: String(row.unit || "").trim(),
              }))
              .filter((row) => row.inventoryItemName && row.quantity > 0)
          : [],
      }));
  ...
  // Recipe / ingredient mapping — a flat list (unlike customizations' nested
  // groups) of { inventoryItemName, quantity, unit } rows. Each ingredient's
  // `inventoryItemName` must match an inventory item's doc id exactly; on
  // order accept, inventoryRecipeService deducts quantity * order quantity
  // from that inventory item automatically (see acceptOrderAndDeduct in
  // services/orderService.js).
  const handleAddRecipeRow = () => {
    setEditedItems((current) => {
      const draft = current[0] || {};
      const rows = Array.isArray(draft.recipe) ? draft.recipe : [];
      return [
        {
          ...draft,
          recipe: [
            ...rows,
            { id: generateRecipeRowId(), inventoryItemName: "", quantity: "", unit: "" },
          ],
        },
      ];
    });
  ...
  const handleRemoveRecipeRow = (rowIndex) => {
    setEditedItems((current) => {
      const draft = current[0] || {};
      const rows = Array.isArray(draft.recipe) ? draft.recipe : [];
      return [{ ...draft, recipe: rows.filter((_, idx) => idx !== rowIndex) }];
    });
  };

  const handleRecipeRowChange = (rowIndex, field, value) => {
    setEditedItems((current) => {
      const draft = current[0] || {};
      const rows = Array.isArray(draft.recipe) ? draft.recipe : [];
      ...
      {isEditingMenu && (
        <section className="menu-editor-panel" aria-label={editModeLabel}>
          ...
          <div className="menu-editor-grid">
            ...
            <div className="menu-editor-field menu-recipe-field">
              <span>Recipe / Ingredients (for auto stock deduction)</span>
              <div className="menu-customizations-builder">
                {(activeEditItem.recipe || []).length === 0 && (
                  <p className="menu-customizations-empty">
                    No ingredients mapped yet. Add one per ingredient this
                    item consumes — the name must match an Inventory item
                    exactly. When a kitchen accepts an order for this item,
                    that quantity is deducted from stock automatically.
                  </p>
                )}
                {(activeEditItem.recipe || []).map((row, idx) => (
                  <div className="menu-recipe-row" key={row.id}>
                    <input
                      className="menu-editor-control menu-customization-option-name"
                      type="text"
                      value={row.inventoryItemName}
                      onChange={(e) =>
                        handleRecipeRowChange(idx, "inventoryItemName", e.target.value)
                      }
                    ...
                    <input
                      ...
                      value={row.quantity}
                      onChange={(e) =>
                        handleRecipeRowChange(idx, "quantity", e.target.value)
                      }
