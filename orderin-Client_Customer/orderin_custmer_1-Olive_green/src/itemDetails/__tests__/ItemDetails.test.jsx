...
describe('ItemDetails', () => {
  ...
  it('renders the item title, description and total price from router state', () => {
    renderDetails();
  ...
  it('adds the item to the cart through the instructions modal', async () => {
    ...
    // instructions modal opens; confirm without notes
    await user.click(screen.getByRole('button', { name: 'Cancel' }));

    expect(addToCart).toHaveBeenCalledWith(item, 1, '', expect.anything());
  });
