...
const loadActivePromotions = async () => {
  try {
    const snap = await getDocs(collection(db, "Restaurant", "orderin_restuarant_5", "promotions"));
...
function Menu({ setIsLoading }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const snap = await getDoc(doc(db, "Restaurant", "orderin_restuarant_5"));
  ...
  useEffect(() => {
    ...
    const fetchMenu = async () => {
      try {
        setIsLoading(true);
        const snap = await withTimeout(getDocs(collection(db, "Restaurant", "orderin_restuarant_5", "menu")), 8000);
