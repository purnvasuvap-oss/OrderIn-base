...
export const verifyMainLogin = async (username, password) => {
  ...
  const userDocRef = doc(db, "Restaurant", "orderin_restuarant_5", "accessControl", "roles", "mainLogin", username);
  try {
...
export const getRestaurantStatus = async () => {
  const restRef = doc(db, "Restaurant", "orderin_restuarant_5");
  try {
...
export const verifySectionPasscode = async (sectionName, passcode) => {
  const sectionRef = collection(db, "Restaurant", "orderin_restuarant_5", "accessControl", "roles", sectionName);
